...
def getDiemSo(im):
    ...
    #............ tim contours co gia tri nho loai bo
    ...
    for cnt in contours:
        if(cv2.arcLength(cnt,True)<lengthCnt):
                #to lai mau den => loai bo
             #cv2.imshow("ss",image_threshold)
             #cv2.waitKey(0)
    ...
    #    return getDiemSo(im)
    #print diemso
    if(len(diemso)>2 and diemso[0] != '1'):
        diemso = diemso[:2]
    if(len(diemso)>1):
        diemso = diemso[:len(diemso)-1] + '.' + diemso[len(diemso)-1:]
